                        ...
                        if IsPedAPlayer(targetPed) then
                            ...
                            if targetId then
                                ...
                                if pedId and (pedId > 0) then
                                    print('1')
                                    TriggerServerEvent('m93-logs:aimlogs', pedId)
                                end
                            end
                        end
                    end
	...
	while true do
		...
		if IsEntityDead(PlayerPedId()) then
			...
			if DeathReason == 'committed suicide' or DeathReason == 'died' then
				TriggerServerEvent('m93-logs:killlogs', GetPlayerName(PlayerId()) .. ' ' .. DeathReason .. '.', Weapon)
			else
				TriggerServerEvent('m93-logs:killlogs', GetPlayerName(Killer) .. '['..GetPlayerServerId(Killer).. '] ' .. DeathReason .. ' ' .. GetPlayerName(PlayerId()) .. '['..GetPlayerServerId(PlayerId())..']', Weapon)
			end
			Killer = nil
			DeathReason = nil
			DeathCauseHash = nil
